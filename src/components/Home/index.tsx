...
import {IKeyEvent, IMenuItem} from '../../utils/types';
...
export const HomeScreen = ({navigation, route}: HomeScreenProps) => {
  ...
  const carouselReRef = useRef<CarouselProps>({
    active: 0,
    items: [],
  });
  ...
  const ListenKeyBoard = (keyEvent: IKeyEvent) => {
    ...
    if (keyMapRef.current.rightKeyCode?.includes(keyEvent.keyCode)) {
      ...
      if (
        carouselReRef.current.active <
        carouselReRef.current.items.length - 1
      ) {
        carouselReRef.current.active = carouselReRef.current.active + 1;
      } else {
    ...
    if (keyMapRef.current.P1_A?.includes(keyEvent.keyCode)) {
      if (route.name == 'Home') {
        const currentItem =
          carouselReRef.current.items[carouselReRef.current.active];
        ...
        <LinearGradient
          ...
          {carousel.items.length ? (
            <View
              ...
              <Animated.View style={currentSlide.current}>
                <Item
                  item={
                    carouselReRef.current.items[carouselReRef.current.active]
                  }
